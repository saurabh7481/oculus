...
room.awareness.updateCursor({ x: 220, y: 180 }, { name: "Ada", color: "#2563eb" });
room.awareness.updateSelection(["node_1"]);

await room.list("layers").move(2, 0);
await room.transaction("Move selected nodes", (tx) => {
  tx.collection<WorkflowNode>("nodes").change("node_1", { x: 240, y: 190 });
});
await room.comments().create("comment_1", { targetId: "node_1", body: "Check this step" });
await room.assets().create("asset_1", { kind: "image", url: "https://assets.example/diagram.png" });
await room.undo();
...
- Transactions group many low-level operations into one labeled user intent for replay and undo.
- Awareness helpers cover live cursors, viewport, selection, active tool, and editing target.
...
Use transactions when one user action produces multiple low-level operations: moving a selection, grouping layers, duplicating workflow nodes, or connecting graph edges. Transactions are applied atomically as one mutation batch, recorded with metadata, and can be undone by the author.

```ts
await room.transaction("Move selected nodes", (tx) => {
  tx.collection<WorkflowNode>("nodes").change("node_1", { x: 240, y: 180 });
  tx.collection<WorkflowNode>("nodes").change("node_2", { x: 420, y: 180 });
}, {
  kind: "move",
  targetIds: ["node_1", "node_2"]
});

if (room.canUndo()) await room.undo();
if (room.canRedo()) await room.redo();
```

`room.transaction(label, operationsOrBuilder, options?)` accepts either an operation array or a builder callback. The builder exposes the same `collection`, `list`, and `tree` helpers as the room, but it stages operations until the callback finishes.

Transaction metadata:

- `label`: human-readable intent shown in replay/debug UI.
- `kind`: optional app-language category such as `"move"`, `"create"`, `"connect"`, or `"delete"`.
- `targetIds`: optional affected object ids.
- `inverseOperations`: generated by the SDK for undo and stored with event metadata.

Undo/redo is per client and per author. Shared/global undo is intentionally deferred because product semantics differ across apps.

## Awareness

Awareness is typed presence for ephemeral collaboration state. It is not persisted in snapshots or replay.

```ts
room.awareness.updateCursor({ x: 200, y: 400 }, { name: "Ada", color: "#2563eb" });
room.awareness.updateViewport({ x: 0, y: 0, zoom: 1.25, pageId: "page_1" });
room.awareness.updateSelection(["shape_1", "shape_2"]);
room.awareness.updateTool("connector");
room.awareness.updateEditing("nodes.node_1.label");
```

These helpers use the same presence transport as `presence.update`, but give whiteboards, design canvases, and workflow builders a shared vocabulary for cursors, viewports, selections, tools, and editing targets.

## Comments

Comments are normal collection records with a small SDK helper for common annotation flows. They work for canvas coordinates, object targets, and path targets.

```ts
const comments = room.comments("comments");

await comments.create("comment_1", {
  targetId: "shape_1",
  body: "This card needs a clearer label"
});

await comments.reply("comment_1", "comment_2", {
  body: "Updated the label"
});

await comments.resolve("comment_1");
await comments.reopen("comment_1");
```

The helper stores `id`, `body`, optional target fields, `authorId`, `parentId`, `resolved`, `createdAt`, and `updatedAt`. It does not impose UI semantics; apps can render threads, pins, or review panels however they like.

## Asset References

Oculus stays storage-agnostic. Store binary files in your own object storage, then sync asset metadata through room state.

```ts
const assets = room.assets("assets");

await assets.create("asset_1", {
  kind: "image",
  url: "https://cdn.example.com/uploads/hero.png",
  width: 1200,
  height: 800,
  mimeType: "image/png",
  metadata: { alt: "Product hero" }
});

await assets.updateMetadata("asset_1", { alt: "Updated alt text" });
await assets.remove("asset_1");
```

Asset references are useful for whiteboard images, design-canvas imports, icons, fonts, and workflow attachments. The engine stores metadata, not bytes.

## Canvas Shapes

Shape helpers provide a shared recipe for whiteboard/design-canvas records while still using ordinary collection operations.

```ts
const shapes = room.shapes("shapes");

await shapes.create("shape_1", {
  type: "rectangle",
  x: 100,
  y: 140,
  width: 240,
...
await shapes.move("shape_1", 180, 220);
await shapes.resize("shape_1", 320, 160);

await shapes.freehand("stroke_1", {
  points: [{ x: 0, y: 0 }, { x: 6, y: 12 }],
  style: { stroke: "#111827", strokeWidth: 2 }
});
```

For high-frequency drawing, batch points into fewer transactions instead of sending one mutation per pointer event.
...
const diff = await room.diffVersions(2, 5);
```
...
### `room.diffVersions(fromVersion, toVersion)`

Fetches a replay debugger diff between two room versions.

```ts
const diff = await room.diffVersions(2, 5);

diff.changedPaths; // ["nodes.node_1.x", "nodes.node_1.label"]
diff.changes; // before/after values with "added", "removed", or "changed"
diff.events; // event-log entries replayed between those versions
```

Diffs are reconstructed from the same snapshot plus event-log path as `replayAt`. This makes them useful for version history, audit panels, and debugging optimistic or offline mutations.
...
### Reusable Room Configuration

Servers can group collection strategies, permission rules, and workflow edge validation in a room definition instead of hardcoding each concern separately.
...
coordinator.defineRoom("workflow_123", {
  ...
  permissions: [
    { path: "nodes.*", operations: ["set"], roles: ["editor", "admin"] },
    { path: "edges.*", operations: ["set", "delete"], roles: ["editor", "admin"] }
  ],
  edgeValidators: [
    {
...
Edge validators reject graph edges whose source/target nodes or ports do not exist before the mutation changes room state or enters the event log.

Room definitions can also include a server-side Zod schema. Unlike the SDK's optimistic client schema, server-side schema validation rejects invalid mutations before they are persisted or broadcast.
...
- `transaction(label, operations, options?)`: labeled mutation batch helper.
- `undo()`: undo the current client's last reversible transaction.
- `redo()`: redo the current client's last undone transaction.
- `updateCursor(cursor, data?)`: typed cursor awareness helper.
- `updateViewport(viewport)`: typed viewport awareness helper.
- `updateSelection(ids)`: typed selection awareness helper.
- `updateTool(tool)`: typed active-tool awareness helper.
- `updateEditing(path)`: typed editing-target awareness helper.
- `updatePresence(data, options?)`: presence helper.
- `replayAt(version)`: replay helper.
- `diffVersions(fromVersion, toVersion)`: replay debugger helper with changed paths, before/after values, and event details.
...
- `MutationMetadata`: transaction/event metadata such as label, kind, target ids, and inverse operations.
- `TransactionOptions`: options for `room.transaction`.
- `CursorAwareness`: cursor coordinate payload.
- `ViewportAwareness`: viewport coordinate and zoom payload.
- `OculusComment`: comment/annotation record shape.
- `AssetReference`: storage-agnostic asset metadata record shape.
- `CanvasShape`: shared shape record for whiteboard/design-canvas recipes.
- `CanvasPoint`: point record for freehand strokes.
- `ReplayDiff`: version-to-version replay diff with changed paths, before/after values, replayed events, and reconstructed states.
- `ReplayDiffChange`: one changed path in a replay diff.
